...
SERVER_PATH_OUTPUTS="/Users/zeleninam2/Documents/1_projects/1_FACE_PAIN/proj_fex_software_comparison/outputs/outputs_pyfeat_svm/"
export SERVER_PATH_OUTPUTS

# csv file to keep track of frames
frames_file="$SERVER_PATH_OUTPUTS/frames_pyfeat_$WHICHPYFEAT-batch-$START-$END.csv"
export frames_file
echo "name, total_frames, processed_frames" > $frames_file

SERVER_PATH_INPUTS='/Users/zeleninam2/Documents/1_projects/1_FACE_PAIN/proj_fex_software_comparison/mydata/testdata'
export SERVER_PATH_INPUTS
...
for folder in "$@"; do
  ...
  bash "$SCRIPT" "$folder" "$WHICHPYFEAT" "frames_file" "$SERVER_PATH_INPUTS" "$SERVER_PATH_OUTPUTS"
  ((i++))
done
' bash "${BATCH_FOLDERS[@]}"
